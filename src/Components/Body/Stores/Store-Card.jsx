...
function StoreCard(props) {
  ...
  return (
    <div>
      <div className="card card-body pb-4 px-6 bg-white">
        <h2 className="flex items-center gap-1 font-semibold text-sm text-opacity-60">
          <IconContext.Provider
            value={{ className: 'fill-border', size: '1rem' }}
          >
            <FaLocationDot />
          </IconContext.Provider>
          Alamat pengiriman
        ...
        <p className="text-sm">
          Kali Guji Kulon, RT 01 RW 03 Desa Rejowinangun, Kemiri, Kab.
          Purworejo, Jawa Tengah, 54262
        </p>
        <button
      ...
      <div className="card card-side bg-white rounded-b-none rounded-t-xl mt-6 px-5 z-10">
        ...
        <div className="card-body rounded-t-xl bg-white pl-2.5">
          ...
          <div className="card-actions justify-end bg-transparent">
            <button
              className="flex items-center gap-1 text-sm text-second"
              onClick={onClick}
            >
              Detail Pesanan
              <IconContext.Provider
      ...
      {isOpenAddress && createPortal(<Address />, document.body)}
    </div>
  );
}
